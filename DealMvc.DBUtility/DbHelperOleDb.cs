...
namespace DealMvc.DBUtility
{
    ...
    /// 数据访问基础类(基于OleDb)  // access -- sql语句的参数顺序必须和OleDbParameter中的顺序一致,否则会出现返回0的问题
    /// 可以用户可以修改满足自己项目的需要。
    /// </summary>
    public abstract class DbHelperOleDb
    {
        ...
        /// <summary>
        /// 执行查询语句，返回OleDbDataReader ( 注意：调用该方法后，一定要对OleDbDataReader进行Close ) --OleDbDataReader myReader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
        /// </summary>
        ...
        public static void ExecuteSqlTran(Dictionary<string, object> SQLStringList)
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                conn.Open();
        ...
        {
            //access不能识别;select @@IDENTITY
            SQLString = SQLString.Replace(";select @@IDENTITY", "");
        ...
        private static void PrepareCommand(OleDbCommand cmd, OleDbConnection conn, OleDbTransaction trans, string cmdText, OleDbParameter[] cmdParms)
        {
            ...
            //调整OleDbParameter[]中的参数顺序,达到和cmdText语句中的参数顺序一致
            cmdParms = ChangeOleDbParametersOrder(cmdText, cmdParms);
        ...
        /// <param name="cmdParms"></param>
        /// <returns></returns>
        private static OleDbParameter[] ChangeOleDbParametersOrder(string cmdText, OleDbParameter[] cmdParms)
        {
            if (cmdParms == null) return cmdParms;

            Regex R = new Regex(@"@\w+", RegexOptions.Multiline | RegexOptions.IgnoreCase);
            MatchCollection _MatchCollection = R.Matches(cmdText);

            OleDbParameter[] _OleDbParameter = new OleDbParameter[Math.Min(cmdParms.Length, _MatchCollection.Count)];

            string Mhas = "";
            int u = 0;
            foreach (Match M in _MatchCollection)
            {
                var canName = M.Value.ToLower();
                if (Mhas.IndexOf(canName) >= 0) break;

                foreach (OleDbParameter _OP in cmdParms)
                {
                    if (_OP.ParameterName.ToLower() == canName)
                    {
                        _OleDbParameter[u] = _OP;
                        u++;
                        Mhas += canName + ",";
                        break;
                    }

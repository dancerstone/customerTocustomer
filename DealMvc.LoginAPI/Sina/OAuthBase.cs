...
namespace DealMvc.API.Sina
{
    ...
    public class OAuthBase
    {
        ...
        /// {帮助函数计算出一个哈希值}
        /// </summary>
        /// <param name="hashAlgorithm">The hashing algoirhtm used. If that algorithm needs some initialization, like HMAC and its derivatives, they should be initialized prior to passing it to this function{哈希algoirhtm的使用。如果需要一些初始化算法,如HMAC及其衍生物,他们应该初始化之前,这个功能的方式}</param>
        /// <param name="data">The data to hash{数据充分}</param>
        /// <returns>a Base64 string of the hash value{一个Base64哈希值线}</returns>
